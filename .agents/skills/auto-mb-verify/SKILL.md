...
- Web changes: run web typecheck, component tests, production build, and relevant Playwright tests. The Playwright axe suite (`pnpm --filter @auto-mb/web test:e2e`) is the standing accessibility gate — it scans real renders in both themes, and it is the only accessibility measurement this repository has. Never state a contrast ratio that was not measured on a real render.
