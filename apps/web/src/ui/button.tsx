...
/* The mock's full ladder. Four text heights (24/28/32/36px) each paired
 * with a square icon-only twin, so a control strip can put an icon button
 * beside a labelled one and have the two agree on height without either
 * being nudged by a caller.
 *
 * The two smallest steps round tighter than `rounded-lg`, and they say so
 * as `min(var(--radius-md), Npx)` rather than a flat value: that is the
 * mock's own expression, and it keeps the corner from outgrowing the box
 * if `--radius` is ever raised.
 *
 * `has-data-[icon=…]` is the mock's asymmetric padding. An icon is
 * visually lighter than a word, so a button that opens with one
 * ("⬆ Upload LOA") looks over-padded on the icon side at the symmetric
 * inset. Marking the icon `data-icon="inline-start"` pulls that edge in by
 * 2px and the button reads as evenly inset. It is opt-in per icon, so
 * nothing that does not carry the attribute moves. Logical names, not
 * left/right: this product renders Devanagari and will render RTL. */
const BUTTON_SIZES = {
  default:
  ...
  'icon-lg': 'size-9',
  /** A button that reads as a link inside running text or a table cell: no
   * box of its own, and free to wrap with the line. */
  inline: 'h-auto p-0 whitespace-normal',
} as const;

export interface ButtonVariantProps {
  readonly variant?: keyof typeof BUTTON_VARIANTS | null | undefined;
  readonly size?: keyof typeof BUTTON_SIZES | null | undefined;
  readonly className?: string | undefined;
}

/** The recipe on its own, for the handful of places that need a real
 * anchor rather than a button and only want the paint. */
