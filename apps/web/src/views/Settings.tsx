 ...
 * as four sibling panels rather than as one page, so each of them repeats
 * the column the way they already each repeated the retired 26rem one —
 * the alternative is a shared export that would pull this module into all
 * four lazy chunks and pay for it at `pnpm bundle:check`. */
const SETTINGS_COLUMN = 'mx-auto w-full max-w-4xl';
...
export function Settings({ api, organisationId, isOwner }: SettingsProps) {
  ...
  if (profile === null) {
    return (
      <Card className={SETTINGS_COLUMN}>
        {/* The literal heading is load-bearing beyond the eye: the
         * loading-anchor census reads it out of this return block to
         * prove "Settings" is visible while the screen loads, and its
         * canary fails if a component hides the string
         * (`test/views/loading-anchor-census.test.ts`). */}
  ...
  return (
    <div className={`${SETTINGS_COLUMN} flex flex-col gap-4`}>
      <PageHeader
        eyebrow="Administration"
        title="Settings"
        description="Manage your organisation, its documents and how they are numbered."
      />

      <Card>
        <CardHeader>
          <div className="flex flex-col gap-1">
            <h2 className="text-base leading-snug font-medium">Company details</h2>
            <p className="text-sm text-muted-foreground">
              Shown at the top of Delivery Challans, invoices and quotations.
            </p>
          </div>
        </CardHeader>
            ...
            <FieldRow>
              ...
              <Field>
                ...
                <Hint id="org-state-code-hint">
                  Two digits; must match the first two of the GSTIN. It decides
                  CGST+SGST against IGST on tax invoices.
                </Hint>
                {stateCodeError !== null && (
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col gap-1">
            <h2 className="text-base leading-snug font-medium">E-invoicing</h2>
            <p className="text-sm text-muted-foreground">
              Whether invoices must be reported to the IRP. E-invoicing is mandatory
              once aggregate turnover has ever crossed ₹5 crore, permanently; from 1
              April 2025, taxpayers at ₹10 crore or more cannot report an invoice more
              than 30 days after its date. Each invoice freezes its reporting deadline
              when it is submitted, and nothing is ever sent automatically.
            </p>
          </div>
        </CardHeader>
        {isOwner ? (
      ...
      <Card>
        <CardHeader>
          <div className="flex flex-col gap-1">
            <h2 className="text-base leading-snug font-medium">Number series</h2>
            <p className="text-sm text-muted-foreground">
              How each document numbers itself. A document you have not configured uses
              the product default, shown here so you can see what your numbers will look
              like. Changing a series never renumbers anything already issued.
            </p>
          </div>
        </CardHeader>
        {seriesError ? (
            ...
            >
              ...
              <Hint>
                ...
                {(
                  ...
                  .map((token) => `{${token}}`)
                  .join(', ')}
                . Use <code>{'{SEQ:3}'}</code> to pad the counter to three digits —
                every format must use <code>{'{SEQ}'}</code>, or every document would
