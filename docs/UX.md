...
**Path segments, not `?tab=`.** This build's fragment carries no query
string anywhere — `parseWorkspaceHash` splits on `/` and decodes — and the
Work page and Masters both address a section as a segment. One serializer,
one grammar; a query parameter here would be a second one for a single
screen.
