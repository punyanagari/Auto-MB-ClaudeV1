...
CREATE TABLE payroll_runs (
  ...
  -- The statutory rates and ceilings this run was computed against,
  -- SNAPSHOTTED at calculate time — the same reason every rate a payslip
  -- rests on is snapshotted onto the line. Without it the run's evidence
  -- panel would re-read the org-editable schedule at display time, and an
  -- owner correcting a notification would restate a finalised run's
  -- evidence while its line figures stayed old. Null until the run is
  -- first calculated, when there is nothing to show. NOT the arithmetic —
  -- the money is on the lines — this is the CA-facing basis panel.
  statutory_basis jsonb,
...
-- ---------------------------------------------------------------------
-- 4b. Maker-checker does not apply to a SALARY request.
--
-- 0080's guard refuses a decision made by the person who raised the
-- request: "somebody other than the claimant agrees to the money." That
-- is exactly right for an advance or a reimbursement, which ARE a
-- person's own claim on the organisation's money.
--
-- A salary request is not that. It is generated by finalising a payroll
-- run — a computed, immutable, gap-free-numbered issued document — and
-- the finaliser is not asking to be paid. The artifact a second person
-- reviews is the RUN, before it is finalised; requiring a DIFFERENT
-- person to then release each of forty salaries is an assumption about
-- org size, and in a single-manager agency it does not merely inconvenience
-- — it makes the salaries impossible to approve OR to reject, because the
-- one manager is the finaliser recorded as requester (ADR-0011: no
-- service account, so the requester is a real user and it is them). That
-- is the S2 aggravator, and it also blocks the cancel path from closing
-- the requests a cancelled run raised.
--
-- So the maker-checker refusal is scoped to the kinds it is actually for.
-- Every other clause of the guard is unchanged — the amount freeze, the
-- decide-once and pay-once races, the terminal states. Restated in full
-- because a trigger function is replaced whole; the ONLY change from
-- 0080 is `AND OLD.kind <> 'salary'` on the maker-checker block.
CREATE OR REPLACE FUNCTION app_private.guard_payment_request_write()
RETURNS trigger
LANGUAGE plpgsql
VOLATILE
...
BEGIN
  ...
  -- MAKER-CHECKER, for a personal claim only. A salary request is a run
  -- obligation, not the finaliser's own claim, so it is exempt — see the
  -- section header. Advances and reimbursements keep the rule.
  IF OLD.kind <> 'salary'
...
BEGIN
  ...
  LOOP
    ...
    -- HOW MANY LADDERS COVER THIS MONTH, and it must be exactly one.
    --
    -- The sum below runs over EVERY row whose window covers the month, so
    -- two ladders that both cover it — the ordinary way that happens is a
    -- new Finance Act recorded without end-dating the old one — would have
    -- their bands summed together and DOUBLE the year's tax on a finalised
    -- issued document. `payroll_statutory_value` and the profession-tax
    -- branch above both refuse an ambiguous cover for exactly this reason;
    -- the income-tax ladder is a set of rows sharing one effective_from
    -- rather than a single value, so the count is over DISTINCT
    -- effective_from. Zero is no ladder; more than one is an overlap.
    SELECT count(DISTINCT s.effective_from)
      INTO v_ladder_versions
    ...
    IF v_ladder_versions = 0 THEN
      RAISE EXCEPTION
        ...
        USING ERRCODE = '23H01';
    END IF;
    IF v_ladder_versions > 1 THEN
      RAISE EXCEPTION
        'the % regime slab ladder for a % taxpayer has % versions in force on %; end-date all but one',
        v_line.tax_regime, v_age_category, v_ladder_versions, v_run.period_month
        USING ERRCODE = '23H01';
    END IF;
    ...
    -- a negative width, which `greatest(…, 0)` drops. Exactly one ladder
    -- covers the month (the count above), so this sums one ladder's bands.
    SELECT coalesce(sum(
             greatest(
  ...
  -- Snapshot the basis alongside the timestamp. The rows in force on the
  -- run's month, ordered so the panel reads the same every time, frozen
  -- here so a later schedule edit cannot restate this run's evidence.
  -- Each parameter resolved to exactly one row at the top of this
  -- function (payroll_statutory_value refuses otherwise), so this cannot
  -- capture a duplicate.
  UPDATE payroll_runs
      ...
      statutory_basis = (
        SELECT coalesce(jsonb_agg(
                 jsonb_build_object(
                   ...
                   'value', b.value::text,
                   'effectiveFrom', b.effective_from::text,
                   'notification', b.notification
                 ) ORDER BY b.parameter
               ), '[]'::jsonb)
        FROM payroll_statutory_rates b
        WHERE b.organisation_id = v_run.organisation_id
          AND b.effective_from <= v_run.period_month
          AND (b.effective_to IS NULL OR b.effective_to >= v_run.period_month)
      )
  WHERE id = p_run_id;
