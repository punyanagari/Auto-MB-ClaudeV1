...
// Every other JSON and shell file in the tree is already parsed by a gate
// that runs FIRST and refuses harder:
//   - `prettier --check .` is the first step of `pnpm verify` and cannot
//     format a file it cannot parse, which covers every JSON file except
//     the ones .prettierignore lists;
//   - ci.yml's shell-lint step runs `bash -n` AND `shellcheck` over
//     `git ls-files '*.sh'` in this same job — a strict superset of a
//     `bash -n` over scripts/ and docker/.
// Only the one file prettier is told to skip is parsed here.
try {
  JSON.parse(
    await readFile(
      new URL('docs/reference/IMPORT-MANIFEST.json', `file://${root}/`),
      'utf8',
    ),
  );
} catch (error) {
  errors.push(`docs/reference/IMPORT-MANIFEST.json: ${String(error)}`);
}
